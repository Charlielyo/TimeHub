...
(function() {
  ...
  class PomodoroTimer {
    ...
    initElements() {
      ...
      // 调试：记录关键元素状态
      console.log('番茄钟元素初始化完成：');
      console.log('- startBtn:', elements.startBtn ? '找到' : '未找到');
      console.log('- modeTabs:', elements.modeTabs ? '找到' : '未找到');
      console.log('- panelTabs:', elements.panelTabs ? `找到 ${elements.panelTabs.length} 个` : '未找到');
      console.log('- taskList:', elements.taskList ? '找到' : '未找到');
      console.log('- whiteNoiseGrid:', elements.whiteNoiseGrid ? '找到' : '未找到');
      console.log('- zenBtn:', elements.zenBtn ? '找到' : '未找到');
      console.log('- exportDataBtn:', elements.exportDataBtn ? '找到' : '未找到');
      console.log('- importDataBtn:', elements.importDataBtn ? '找到' : '未找到');
      console.log('- resetDataBtn:', elements.resetDataBtn ? '找到' : '未找到');
    }
    ...
    exportData() {
      ...
      if (!TimeHubUtils) {
        console.error('TimeHubUtils未定义');
        alert('工具库未加载，请刷新页面重试');
        return;
      }
      const data = {
        tasks: this.tasks,
